<?php
    ...
    function lecturas(){
        ...
        */
        ...
        '*',
        'ATC_LECTURA',
        'WHERE TRUNC(FECHACREADO) = TRUNC(SYSDATE) - 5 
    ...
    # Obtiene la cantidad de comunicados
    $cantidadLecturas = count($resultadoLecturas);
    #echo "Se crearán: " . $cantidadLecturas . " arreglos <br/>";
    #die();

    # Obtiene la data con base al id
    ...
    function crearTablaGerentesCorreo($dataGerentes){
     ...
     function email($titulo, $fecha, $tablaRegionesCorreo, $tablaGerentesCorreo, $correo){
		...
		$mail->SMTPAuth   = true;
		$mail->SMTPSecure = "tls";
	    #$mail->Host     = "smtp.izzi.mx";
	    $mail->Host       = "172.21.32.84";
		$mail->Port       = '587';
		$mail->Username   = "Notificatiendas@izzi.mx";
		#$mail->Password   = '6AuSrXILpUW!#%)h';
		$mail->Password   = 'L8yPR7T5nFzGMLl/';
		$mail->From       = "Notificatiendas@izzi.mx";
		// Fin Configuración Nueva //

		$mail->FromName = "cx tiendas";
		
		$mail->Subject  = 'Reporte de lectura';
		$mail->AltBody  = "--";

        $mensaje = "
            <div style='font-family: Arial, sans-serif; max-width: 1200px; margin: auto;'>
                <div style='display: flex; justify-content: space-between; align-items: center;'>
                    <h3 style='margin: 0;'>Lectura: ". htmlspecialchars($titulo) ."</h3>
                    <h3 style='margin: 0;'>Enviado: ". htmlspecialchars($fecha) ."</h3>
                </div>

                <hr style='border: 1px solid #DDDDDD; margin: 10px 0;'>
                ...
                <div style='margin-top: 40px;'>
                 ...
                 <div style='margin-top: 40px;'>
                    <a style='color: #C30A4A;' href='https://cxtiendas.izzi.mx/pruebas/reportelectura/reporte_excel_regiones.php'>
                ...
                <div style='margin-top: 40px;'>
                    <a style='color: #C30A4A;' href='https://cxtiendas.izzi.mx/pruebas/reportelectura/reporte_excel_gerentes.php'>
                        Lectura por Gerentes ATC
                    </a>
                </div>
            </div>
        ";
		
		// $mail->MsgHTML($tablaRegiones . "<hr/>" . $tablaGerentes);
		$mail->MsgHTML($mensaje);
		
		#$mail->AddAddress("$correo");

        # Recorre el arreglo de gerentes a los cuales será enviado el correo
        if(!empty($correo)){
            foreach($correo as $email){
                $mail->AddAddress($email);
            }
        } else {
            die('Error: No hay direcciones de correo válidas');
        }
		
		//$mail->AddBCC("asaucedo@izzi.mx","asaucedo");
		...
		if (!$exito){
			...
			echo "<div align='center'>Ocurrió un problema, el correo electrónico no ha sido enviado: ".$exito." "."reportalo a: asaucedo@izzi.mx</div>";
            ...
            # Crea un archivo con los datos del correo enviado
            /*
            $archivo = "correos_enviados/" .date("Y-m-d_H-i-s") . ".txt";
            file_put_contents($archivo, $titulo);
            */
		}
    ...
    // Envío de correo electrónico
    function emailDos($titulo, $fecha, $tablaRegionesCorreo, $tablaGerentesCorreo, $correo){
    
        // Nombre del archivo donde se almacenan los títulos de correos enviados
        $archivo = "correos_enviados/correos_enviados.txt";
    
        // Verificar si el archivo existe y leer su contenido
        if (file_exists($archivo)) {
            $titulos_enviados = file($archivo, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
        } else {
            $titulos_enviados = [];
        }
    
        // Comprobar si el título ya está en el archivo
        if (in_array(trim($titulo), $titulos_enviados)) {
            echo "Correo no enviado. Ya se ha enviado un correo con este título.";
            return false; // Detenemos la ejecución y evitamos el envío
        }
        ...
        $mensaje = "
            ...
            <hr style='border: 1px solid #DDDDDD; margin: 10px 0;'>
            <div style='margin-top: 40px;'>$tablaRegionesCorreo</div>
            <div style='margin-top: 40px;'>
            <a style='color: #C30A4A;' href='https://cxtiendas.izzi.mx/pruebas/reportelectura/reporte_excel_regiones.php'>Lectura por Regiones</a>
            </div>
            <div style='margin-top: 40px;'>$tablaGerentesCorreo</div>
            <div style='margin-top: 40px;'>
            <a style='color: #C30A4A;' href='https://cxtiendas.izzi.mx/pruebas/reportelectura/reporte_excel_gerentes.php'>Lectura por Gerentes ATC</a>
            </div>
            </div>";
        ...
        if (!$exito) {
            echo "<div align='center'>Ocurrió un problema, el correo electrónico no ha sido enviado. Reportalo a: asaucedo@izzi.mx</div>";
            echo "<br/>".$mail->ErrorInfo;
        } else {
            echo "Correo electrónico enviado correctamente.";
    
            // Guardar el título en el archivo de registros
            file_put_contents($archivo, trim($titulo) . PHP_EOL, FILE_APPEND);
        }
    ...
    for($i = 0; $i < $cantidadLecturas; $i++){
        ...
        # Arreglo actual
        ${"array_" . $i} = obtenerData($idLectura);
        #var_dump(${"array_" . $i});
        ...
        echo "Datos por regiones: <br/>";
        echo "<pre>";
        echo var_dump($dataRegiones);
        echo "</pre>";
        echo "<br/>";
        ...
        echo "Datos por gerentes: <br/>";
        echo "<pre>";
        echo var_dump($dataGerentes);
        echo "</pre>";
        echo "<br/>";
