<?php
    ...
    foreach($data as $elemento){
        foreach($elemento['detalles'] as $detalle){
            $region = $detalle['responsableregion'] ?? 'Desconocido';

            if(isset($regiones[$region])){
                if($detalle['estatus'] === 'Leido'){
                    $regionesLeidos[$region]++;
                } else if($detalle['estatus'] === 'Pendiente'){
    ...
    */
    ...
    foreach($data as $elemento){
        foreach($elemento['detalles'] as $detalle){
            $nombreGerente = $detalle['responsablenombre'] ?? 'Desconocido';
            ...
            # Formatea las fechas
            $fechaCreado = DateTime::createFromFormat('d-M-y h:i:s.u A', str_replace('.', '.', $detalle['fechacreado']));
            $fechaLeido = DateTime::createFromFormat('d-M-y h:i:s.u A', str_replace('.', '.', $detalle['fechaleido']));
    ...
    # Convierte la estructura en un array indexado 
    $dataGerentes = array_values($dataGerentes);

    # Ordena alfabéticamente por nombre del gerente
