...
function calificarVideo(id_video, tiempoEnMin, usuario) {
          ...
          btn.addEventListener('click', async () => {
            ...
            try {
              ...
              if (result.status === 'ok') {
                ...
                const videoActualizado = videos.find(v => v.id_video == id_video);
                if (videoActualizado) {
                ...
                const pendientes = videos.filter(v => v.estado === 'pendiente');
                const vistos = videos.filter(v => v.estado === 'visto');
...
async function actualizarVisualizacion(id_video, tiempo_min, usuario) {
  try {
    await fetch("actualizaciones_dos.php", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
